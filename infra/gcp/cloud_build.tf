...
resource "google_cloudbuild_trigger" "core_trigger" {
  name     = "${var.project_prefix}-core-cloud-build-trigger"
  location = "global"
  ...
  service_account = google_service_account.cloud_build_invoker.email

  depends_on = [google_service_account.cloud_build_invoker, google_service_account.cloud_run_invoker]
}
