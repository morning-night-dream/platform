...
func (*ReadArticle) scanValues(columns []string) ([]interface{}, error) {
...
func (ra *ReadArticle) assignValues(columns []string, values []interface{}) error {
...
func (ra *ReadArticle) QueryArticle() *ArticleQuery {
	return (&ReadArticleClient{config: ra.config}).QueryArticle(ra)
}
...
func (ra *ReadArticle) Update() *ReadArticleUpdateOne {
	return (&ReadArticleClient{config: ra.config}).UpdateOne(ra)
}
...
func (ra ReadArticles) config(cfg config) {
	for _i := range ra {
		ra[_i].config = cfg
	}
}
