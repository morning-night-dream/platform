...
func toProtoUser(e *ent.User) (*User, error) {
	...
	for _, edg := range e.Edges.Auths {
		id, err := edg.ID.MarshalBinary()
		if err != nil {
			return nil, err
		}
		v.Auths = append(v.Auths, &Auth{
			Id: id,
		})
	}
	return v, nil
...
func (svc *UserService) Get(ctx context.Context, req *GetUserRequest) (*User, error) {
	...
	case GetUserRequest_WITH_EDGE_IDS:
		get, err = svc.client.User.Query().
			Where(user.ID(id)).
			WithAuths(func(query *ent.AuthQuery) {
				query.Select(auth.FieldID)
			}).
			Only(ctx)
	default:
...
func (svc *UserService) Update(ctx context.Context, req *UpdateUserRequest) (*User, error) {
	...
	for _, item := range user.GetAuths() {
		var auths uuid.UUID
		if err := (&auths).UnmarshalBinary(item.GetId()); err != nil {
			return nil, status.Errorf(codes.InvalidArgument, "invalid argument: %s", err)
		}
		m.AddAuthIDs(auths)
	}
